        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full" role="dialog" aria-modal="true" aria-labelledby="modal-headline">
            <form>
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="mb-4">
                        <label for="centro_id" class="text-gray-700 text-sm font-bold mb-2">Centro de votación*:</label>
                        <select id="centro_id" wire:model="centro_id" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                            <option selected value="">seleccionar</option>
                            @foreach($centros as $centro)
                                <option value="{{$centro->id}}">{{$centro->id}}&ensp;{{$centro->nombrecentro}}&ensp;sector:&ensp;{{$centro->sector->nombresector}}</option>
                            @endforeach
                        </select>
                        @error('centro_id')
                        <div class="bg-orange-100 border-l-4 border-orange-500 text-orange-700 p-4" role="alert">
